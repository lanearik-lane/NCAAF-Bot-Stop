...
st.caption('All-conference upcoming-game scan • FBS + FCS + D-II/D-III • moneyline model • automatic player-prop targets • no API keys')

with st.sidebar:
    st.header('🤖 Auto Bot')
    auto = st.toggle('Auto-run bot', value=True)
    lead_days = st.slider('Scan games this many days ahead', 1, 14, 7)
    refresh = st.selectbox('Automatic refresh', ['15 minutes', '30 minutes', '60 minutes'], index=1)
    ...
    st.caption('FBS and FCS group scans cover their conferences. D-II/D-III uses ESPN’s combined public scoreboard pool.')
...
def run_scan(force=False):
    ...
    due = last is None or (datetime.now(ET) - last).total_seconds() >= mins * 60
    if force or config_changed or due:
        with st.spinner(f'Scanning every selected division/conference pool for games in the next {lead_days} days...'):
...
if st.button('🔍 SCAN ALL CONFERENCE GAMES + PLAYER PROPS NOW', type='primary', use_container_width=True):
    run_scan(force=True)

if auto:
    st.caption(f"🟢 AUTO BOT ON • next-{lead_days}-days scan • refresh target: {refresh}")
else:
    st.caption('⚪ AUTO BOT OFF • use the scan button')

games = st.session_state.get('games', [])
if 'scanned' not in st.session_state:
    st.info('Turn on Auto-run or press **SCAN ALL CONFERENCE GAMES + PLAYER PROPS NOW**.')
elif not games:
