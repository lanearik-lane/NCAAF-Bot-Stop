...
# Streamlit Cloud normally installs beautifulsoup4 from requirements.txt.
# This fallback self-heals deployments where requirements.txt was not picked up.
def _load_bs4():
    try:
        ...
        try:
            subprocess.check_call(
                [sys.executable, "-m", "pip", "install", "--quiet", "beautifulsoup4>=4.12,<5"]
            )
            ...
            st.error(
                "Missing dependency: beautifulsoup4. Make sure requirements.txt is in the same "
                "GitHub folder as app.py, then reboot the Streamlit app."
            )
            st.code("beautifulsoup4>=4.12,<5", language="text")
            st.exception(exc)
            st.stop()
...
.daily-wrap{border:1px solid #2f3742;border-radius:14px;padding:14px 16px;margin:12px 0 18px;background:#11151b}
.daily-title{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-weight:900;font-size:1rem;margin-bottom:10px}
.daily-row{display:grid;grid-template-columns:38px 92px 1fr 120px;gap:10px;align-items:center;padding:7px 8px;border-radius:9px;margin:3px 0;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
.daily-row.today{background:#18251f;border:1px solid #2d6a4f}
.daily-row.done{color:#b7c2ce}
.daily-row.next{color:#7f8a98}
.daily-mark{font-weight:900;text-align:center}
.daily-state{font-size:.82rem;text-align:right;font-weight:800}
.daily-today{color:#32d583}.daily-done{color:#9aa4b2}.daily-next{color:#7f8a98}
@media (max-width:700px){.daily-row{grid-template-columns:30px 76px 1fr}.daily-state{display:none}}
</style>
""",
...
DAILY_WORKFLOW = [
    ("Sunday", "NEW SLATE", "Find the next slate and upcoming games"),
    ("Monday", "TEAM DATA", "Refresh schedules, team records, ratings, and logos"),
    ("Tuesday", "PLAYER DATA", "Refresh player usage and matchup statistics"),
    ("Wednesday", "MONEYLINE BOARD", "Publish A+ through F moneyline tiers"),
    ("Thursday", "MATCHUP REFRESH", "Re-compare teams and update moneyline grades"),
    ("Friday", "PROP UPGRADE", "Upgrade anytime TD, rushing, receptions, and receiving props"),
    ("Saturday", "FINAL PREGAME", "Final pregame moneyline and player-prop board"),
]


def workflow_index(now):
    # Python Monday=0..Sunday=6.  Our football workflow starts Sunday=0.
    return (now.weekday() + 1) % 7


def weekly_status(now):
    idx = workflow_index(now)
    day, phase, desc = DAILY_WORKFLOW[idx]
    return phase, f"{day} • {desc}"


def render_daily_checklist(now):
    idx = workflow_index(now)
    rows = ["<div class='daily-wrap'><div class='daily-title'>📋 DAILY CHECKLIST</div>"]
    for i, (day, stage, desc) in enumerate(DAILY_WORKFLOW):
        if i < idx:
            cls, mark, state, state_cls = "done", "✓", "COMPLETE", "daily-done"
        elif i == idx:
            cls, mark, state, state_cls = "today", "●", "TODAY", "daily-today"
        else:
            cls, mark, state, state_cls = "next", "○", "UPCOMING", "daily-next"
        rows.append(
            f"<div class='daily-row {cls}'>"
            f"<div class='daily-mark {state_cls}'>{mark}</div>"
            f"<div><b>{day[:3].upper()}</b></div>"
            f"<div><b>{stage}</b><br><span class='muted'>{desc}</span></div>"
            f"<div class='daily-state {state_cls}'>{state}</div>"
            f"</div>"
        )
    rows.append("</div>")
...
if not modeled:
    ...
    # ---------------------- Date checklist / view filter ----------------------
    # This does not change the hands-free scan. The app still loads the full
    # upcoming slate automatically; these checkboxes only control which dates
    # are displayed in the board.
    available_dates = sorted({
        g["date"].date() for g, _, _ in modeled if g.get("date") is not None
    })

    st.subheader("📅 GAME DATE CHECKLIST")
    st.caption("Choose the game dates you want on the board. All available dates are selected by default.")

    selected_dates = []
    if available_dates:
        cols = st.columns(min(4, len(available_dates)))
        for i, game_date in enumerate(available_dates):
            label = game_date.strftime("%a • %b %d")
            with cols[i % len(cols)]:
                if st.checkbox(label, value=True, key=f"game_date_{game_date.isoformat()}"):
                    selected_dates.append(game_date)

    if not selected_dates:
        st.warning("Select at least one game date to display moneyline picks and player props.")
        filtered_modeled = []
    else:
        filtered_modeled = [
            x for x in modeled
            if x[0].get("date") is not None and x[0]["date"].date() in selected_dates
        ]

    anchors = [x for x in filtered_modeled if x[1]["grade"] == "A+"]
    strong = [x for x in filtered_modeled if x[1]["grade"] in ("A", "B+")]
    anchors.sort(key=lambda x: x[1]["p"], reverse=True)
    strong.sort(key=lambda x: x[1]["p"], reverse=True)

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Games Shown", len(filtered_modeled), delta=f"{len(modeled)} total loaded")
    c2.metric("A+ Anchors", len(anchors))
    c3.metric("A / B+ Strong", len(strong))
    c4.metric("Prop Candidates", sum(len(x[2]) for x in filtered_modeled))
    ...
    chronological = sorted(filtered_modeled, key=lambda x: (x[0].get("date") or datetime.max.replace(tzinfo=ET), -x[1]["p"]))
    last_bucket = None
