...
def parse_cbs_schedule_html(html: str, start, end, source_label: str):
    """Parse CBS schedule pages without assuming a specific heading level.

    CBS changes its markup often.  The earlier build only looked for h2/h3 date
    headings, which can return zero games even when the page loaded correctly.
    This parser locates date text nodes anywhere in the document, associates
    them with the next schedule table, and then extracts the first two team
    cells plus the kickoff time.
    """
    games = []
    soup = BeautifulSoup(html, "html.parser")
    date_re = re.compile(
    ...
    def parse_date(txt):
        ...
        if not date_re.match(txt):
            return None
        try:
            return datetime.strptime(txt, "%A, %B %d, %Y").date()
        except Exception:
            return None
    ...
    def parse_row(tr, gd):
        ...
        # Prefer cells that actually link to CBS team pages.  This is more
        # robust than assuming the first two cells forever.
        team_cells = []
        for td in tds:
            if td.find("a", href=re.compile(r"/college-football/teams/", re.I)):
                team_cells.append(td)
        if len(team_cells) >= 2:
            away_cell, home_cell = team_cells[0], team_cells[1]
        else:
            away_cell, home_cell = tds[0], tds[1]

        away_name, away_code, away_logo = cbs_team_from_cell(away_cell)
        home_name, home_code, home_logo = cbs_team_from_cell(home_cell)
        away_name, home_name = clean_team(away_name), clean_team(home_name)
        if not away_name or not home_name:
            return None
        if norm(away_name) in {"away", "team"} or norm(home_name) in {"home", "team"}:
            return None

        row_text = " ".join(tr.stripped_strings)
        mt = re.search(r"\b(TBD|\d{1,2}:\d{2}\s*(?:am|pm))\b", row_text, re.I)
        raw_time = mt.group(1) if mt else "TBD"
        if raw_time.upper() == "TBD":
            kickoff = datetime(gd.year, gd.month, gd.day, 12, 0, tzinfo=ET)
            time_label = "TBD"
        else:
            try:
                parsed = datetime.strptime(f"{gd.isoformat()} {raw_time}", "%Y-%m-%d %I:%M %p")
                kickoff = parsed.replace(tzinfo=ET)
                time_label = parsed.strftime("%-I:%M %p") if hasattr(parsed, "strftime") else raw_time.upper()
            except Exception:
                kickoff = datetime(gd.year, gd.month, gd.day, 12, 0, tzinfo=ET)
                time_label = raw_time.upper()
    ...
    # Find every visible date string regardless of whether CBS renders it as
    # h2, h3, h4, div, etc.  Then take the next table in document order.
    seen_tables = set()
    date_nodes = []
    for txt in soup.find_all(string=True):
        gd = parse_date(txt)
        if gd is not None and start <= gd <= end:
            date_nodes.append((txt, gd))

    for txt, gd in date_nodes:
        parent = txt.parent if getattr(txt, "parent", None) else None
        ...
        for tr in table.find_all("tr"):
            game = parse_row(tr, gd)
            if game:
                games.append(game)

    # Fallback: some CBS variants put the date outside a heading and the table
    # parser may not encounter the exact date text node first.  For each table,
    # look backwards for the nearest date-looking string.
    if not games:
        for table in soup.find_all("table"):
            gd = None
            for prev in table.find_all_previous(string=True, limit=250):
                maybe = parse_date(prev)
    ...
    # De-duplicate in case a responsive desktop/mobile table is duplicated.
    dedup = {}
    for g in games:
        ...
        for t in tables_from_html(html):
            ...
            for _, r in t.iterrows():
                ...
                if key in seen:
                    continue
                seen.add(key)
                rows.append({"player": player, "team": team_code or "Unknown", "team_code": team_code, "team_key": norm(team_code), "kind": kind, "position": pos, "games": gp or 1, "ypg": ypg, "yards": yds, "td": number(r.get(tdc)) if tdc else None, "source": "CBS Sports"})
            break
    return rows
...
def player_props(game, model, core):
    ...
    for kind, market in mapping.items():
        for r in core["players"].get(kind, []):
            tm = None
            # CBS player tables expose team codes; CBS schedule rows expose the same code.
            if r.get("team_code"):
                for obj in team_keys.values():
                    if obj.get("cbs_code") and obj.get("cbs_code").upper() == r.get("team_code", "").upper():
                        tm = obj
                        break
            if tm is None and r["team_key"] in team_keys:
                tm = team_keys[r["team_key"]]
            if tm is None:
                for tk, obj in team_keys.items():
                    if len(tk) >= 4 and r.get("team_key") and (tk in r["team_key"] or r["team_key"] in tk):
                        tm = obj
                        break
            if not tm:
                continue
