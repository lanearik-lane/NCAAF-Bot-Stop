...
def game_card(g):
    ...
    st.markdown(
        ...
        f"Pick Strength&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b>{m['grade']}</b></div>", unsafe_allow_html=True)
...
st.caption('All-conference upcoming-game scan • FBS + FCS + D-II/D-III • moneyline model • player-prop targets • manual refresh • no API keys')

with st.sidebar:
    st.header('🔍 Scan Settings')
    lead_days = st.slider('Scan games this many days ahead', 1, 14, 7)
    selected_divs = st.multiselect('Divisions / conference pools', list(SCAN_GROUPS), default=list(SCAN_GROUPS))
    ming = st.selectbox('Minimum ML model strength', ['ALL', 'B', 'B+', 'A', 'A+'])
    show_passes = st.toggle('Show games with ML PASS', value=True)
    st.caption('No auto bot or recurring refresh. The app performs one scan when it opens, then only scans again when you press the button or change scan settings.')
...
def run_scan(force=False):
    if not selected_divs:
        ...
        return
    config_changed = (st.session_state.get('lead_days') != lead_days or
                      st.session_state.get('selected_divs') != selected_divs)
    first_scan = 'scanned' not in st.session_state
    if force or config_changed or first_scan:
        with st.spinner(f'Searching now: all selected division/conference pools for games in the next {lead_days} days, plus player prop candidates...'):
            gs, errs = scan_dates(datetime.now(ET).date(), lead_days, selected_divs)
            st.session_state.games = gs
            st.session_state.errors = errs
            st.session_state.scanned = datetime.now(ET)
            st.session_state.lead_days = lead_days
            st.session_state.selected_divs = list(selected_divs)


# One immediate search when the app starts. This is not a recurring auto bot.
run_scan()

if st.button('🔍 SEARCH NOW — ALL CONFERENCE GAMES + PLAYER PROPS', type='primary', use_container_width=True):
    run_scan(force=True)

st.caption(f"🔎 MANUAL SCAN MODE • searching next {lead_days} days • no automatic refresh")

games = st.session_state.get('games', [])
if 'scanned' not in st.session_state:
    st.info('The first search has not completed yet. Press **SEARCH NOW — ALL CONFERENCE GAMES + PLAYER PROPS** to retry.')
elif not games:
    st.warning('No upcoming games were parsed in the selected window. No games, players, or lines were fabricated.')
else:
    ranks = {'PASS': 0, 'B': 1, 'B+': 2, 'A': 3, 'A+': 4}
    th = 0 if ming == 'ALL' else ranks[ming]
    qualified = []
    for g in games:
        m = model(g)
        if m is None:
            if show_passes and ming == 'ALL':
                qualified.append((g, None))
        elif ranks[m['grade']] >= th and (show_passes or m['grade'] != 'PASS'):
            qualified.append((g, m))

    c1, c2, c3, c4 = st.columns(4)
    c1.metric('Upcoming Games', len(games))
    c2.metric('Displayed', len(qualified))
    c3.metric('FBS', sum(g['division'] == 'FBS' for g in games))
    c4.metric('FCS + D-II/III', sum(g['division'] != 'FBS' for g in games))
    st.success(f"Last scan: {st.session_state.scanned.strftime('%m/%d/%y %I:%M %p ET')}")

    for g, _ in qualified:
        game_card(g)

errors = st.session_state.get('errors', [])
if errors:
    with st.expander(f'⚠️ Source warnings ({len(errors)})'):
        st.write('Some public pages did not load. The bot kept the games it could verify.')
        st.code('\n'.join(errors[:50]))
...
st.caption(
    ...
    'A sportsbook line is treated as verified only when you enter it in the grader. Public webpage markup can change.'
)
