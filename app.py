...
st.set_page_config(page_title='NCAAF Weekly Edge Scanner', page_icon='🏈', layout='wide')
...
.phase-badge{display:inline-block;border:1px solid #555;border-radius:999px;padding:5px 10px;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-weight:900;margin:4px 0 10px 0}
.phase-early{background:#132238}.phase-updated{background:#332a10}.phase-prop{background:#2c1738}.phase-final{background:#123120}.phase-next{background:#282828}
.timeline{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;border:1px solid #3b3b3b;border-radius:12px;padding:10px 14px;margin:6px 0 14px;background:#171717}
</style>''', unsafe_allow_html=True)


# ---------- weekly workflow / pick maturity ----------
...
def weekly_phase(now=None):
    now = now or datetime.now(ET)
    wd = now.weekday()
    if wd in (0,1):
        return {'name':'NEXT SLATE BUILD','icon':'⚙️','css':'phase-next',
                'desc':'Building the next slate. Team data is collected now; Wednesday becomes the first full moneyline board.'}
    if wd == 2:
        return {'name':'EARLY MODEL','icon':'🟦','css':'phase-early',
                'desc':'Wednesday moneyline board is live. A+ through F moneyline grades are active; player props are preliminary.'}
    if wd == 3:
        return {'name':'UPDATED','icon':'🟨','css':'phase-updated',
                'desc':'Thursday refresh. Team efficiency and matchup data are re-compared and player projections gain more weight.'}
    if wd == 4:
        return {'name':'PROP UPGRADE','icon':'🟪','css':'phase-prop',
                'desc':'Friday prop upgrade. Player projections are emphasized and may reach A+ when source coverage supports it.'}
    if wd == 5:
        return {'name':'FINAL PREGAME','icon':'🟩','css':'phase-final',
                'desc':'Saturday final-pregame board. Strongest moneylines and player props are surfaced first.'}
    return {'name':'NEXT SLATE BUILD','icon':'⚙️','css':'phase-next',
            'desc':'Sunday reset. The app starts building the next week automatically from public data.'}

def game_phase(game, now=None):
    now = now or datetime.now(ET)
    hours = (game['date'] - now).total_seconds()/3600
    if hours <= 0:
        return {'name':'STARTED','icon':'⏱️','css':'phase-next','prop_cap':'F',
                'desc':'Game has started; pregame grades should no longer be treated as current.'}
    if hours <= 18:
        return {'name':'FINAL PREGAME','icon':'🟩','css':'phase-final','prop_cap':'A+',
                'desc':'Final pregame window.'}
    if hours <= 42:
        return {'name':'PROP UPGRADE','icon':'🟪','css':'phase-prop','prop_cap':'A+',
                'desc':'Player-prop upgrade window.'}
    if hours <= 66:
        return {'name':'UPDATED','icon':'🟨','css':'phase-updated','prop_cap':'A',
                'desc':'Updated team and matchup window.'}
    return {'name':'EARLY MODEL','icon':'🟦','css':'phase-early','prop_cap':'B+',
            'desc':'Early moneyline model; player props are intentionally capped until closer to kickoff.'}

def phase_rank(game):
    rank={'FINAL PREGAME':4,'PROP UPGRADE':3,'UPDATED':2,'EARLY MODEL':1,'STARTED':0}
    return rank.get(game_phase(game)['name'],0)
...
def prop_cards(g,m,phase):
    heading = 'PRELIMINARY PLAYER PROPS' if phase['name']=='EARLY MODEL' else ('UPDATED PLAYER PROPS' if phase['name']=='UPDATED' else 'BEST PLAYER PROPS')
    st.markdown(f"<div class='rule'></div><div class='section-title'>⭐ &nbsp;{heading}</div>",unsafe_allow_html=True)
    ...
    for p in props:
        ...
        display_grade=cap_grade(p['grade'],phase.get('prop_cap','A+'))
        maturity_note = '' if display_grade==p['grade'] else f" • Early-window cap: {phase.get('prop_cap')}"
        st.markdown(f"<div class='propbox mono'><b>{p['market']}</b><br>Player&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b>{p['player']} ({p['team']['abbr']})</b><br>"
                    f"Projection&nbsp;&nbsp; <b>{p['projection']:.1f}</b><br>Model target <b>OVER {p['target_over']:.1f} or lower</b><br>Strength&nbsp;&nbsp;&nbsp;&nbsp; <b>{display_grade}</b><br>"
                    f"<span class='muted'>Status: {phase['name']} • Base: {p['base']:.1f}/game • Sources used: {p['source_count']} • {wh}. Target is a model threshold, not a sportsbook line.{maturity_note}</span></div>",unsafe_allow_html=True)
...
def game_card(g):
    ...
    source_badges(m)
    st.markdown(f"<span class='phase-badge {phase['css']}'>{phase['icon']} {phase['name']}</span>",unsafe_allow_html=True)
    ...
    st.markdown(f"<div class='mono'>Win Probability&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b>{m['p']*100:.0f}%</b><br>Model Edge&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b>N/A until verified MLs</b><br>Pick Strength&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b>{m['grade']}</b><br>Model Status&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <b>{phase['name']}</b><br>Independent sources&nbsp;&nbsp; <b>{m['coverage']}</b></div>",unsafe_allow_html=True)
...
# ---------- UI ----------
st.title('🏈 NCAAF WEEKLY EDGE SCANNER')
...
_week=weekly_phase(_now)
st.markdown(f"<span class='phase-badge {_week['css']}'>{_week['icon']} WEEKLY STATUS: {_week['name']}</span>",unsafe_allow_html=True)
st.write(_week['desc'])
st.markdown("<div class='timeline'><b>WED</b> Moneyline Board &nbsp;→&nbsp; <b>THU</b> Updated Matchups &nbsp;→&nbsp; <b>FRI</b> Player Prop Upgrade &nbsp;→&nbsp; <b>SAT</b> Final Pregame</div>",unsafe_allow_html=True)

config_key=(_now.date().isoformat(),SCAN_AHEAD_DAYS,tuple(SELECTED_DIVISIONS),_week['name'])
...
if not games:
    ...
    final_ct=sum(1 for g,_ in modeled if game_phase(g)['name']=='FINAL PREGAME')
    prop_ct=sum(1 for g,_ in modeled if game_phase(g)['name']=='PROP UPGRADE')
    c1,c2,c3,c4,c5=st.columns(5); c1.metric('Upcoming Games',len(games)); c2.metric('A+ Anchors',len(anchors)); c3.metric('A / B+ Strong',len(strong)); c4.metric('Prop Upgrade',prop_ct); c5.metric('Final Pregame',final_ct)
    ...
    if anchors:
        for g,m in sorted(anchors,key=lambda x:(phase_rank(x[0]),x[1]['p']),reverse=True): game_card(g)
    else: st.info('No A+ picks met the multi-source coverage threshold.')
    st.subheader('💪 A / B+ STRONG PICKS')
    if strong:
        for g,m in sorted(strong,key=lambda x:(phase_rank(x[0]),x[1]['p']),reverse=True): game_card(g)
    else: st.info('No A or B+ picks were found.')
    with st.expander(f'📋 B THROUGH F UPCOMING GAMES ({len(others)})'):
        for g,m in sorted(others,key=lambda x:(phase_rank(x[0]),x[1]['p'] if x[1] else 0),reverse=True):
            if m: game_card(g)
...
st.divider()
st.caption('Workflow: Wednesday publishes the first full moneyline board; Thursday re-compares matchup inputs; Friday upgrades player-prop emphasis; Saturday labels games in the final-pregame window. Prop grades are intentionally capped earlier in the week and can rise as kickoff approaches. Data integrity: blocked or missing sources are skipped rather than fabricated. The app does not invent source values, sportsbook lines, players, or edges.')
