...
def parse_cbs_schedule_html(html: str, start, end, source_label: str):
    ...
    for h in soup.find_all(["h2", "h3"]):
        date_text = " ".join(h.stripped_strings).strip()
        if not re.match(r"^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),", date_text):
            continue
        try:
            gd = datetime.strptime(date_text, "%A, %B %d, %Y").date()
        except Exception:
            continue
        if not (start <= gd <= end):
            continue
        ...
        for tr in table.find_all("tr"):
            ...
            if len(tds) < 3:
                continue
            away_name, away_code, away_logo = cbs_team_from_cell(tds[0])
            home_name, home_code, home_logo = cbs_team_from_cell(tds[1])
            if not away_name or not home_name:
                continue
            if away_name.lower() in {"away", "team"} or home_name.lower() in {"home", "team"}:
                continue
            time_text = " ".join(tds[2].stripped_strings).strip()
            mt = re.search(r"(TBD|\d{1,2}:\d{2}\s*(?:am|pm))", time_text, re.I)
            raw_time = mt.group(1).upper() if mt and mt.group(1).upper() == "TBD" else (mt.group(1).lower() if mt else "TBD")
            if raw_time == "TBD":
            ...
            games.append({
                ...
                "time_label": raw_time.upper() if raw_time == "TBD" else raw_time.upper(),
    ...
    with ThreadPoolExecutor(max_workers=2) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if err:
                errors[label] = err
            if html:
                games.extend(parse_cbs_schedule_html(html, start, end, label))
...
if not sp["games"]:
    ...
    st.caption("The app tried CBS Sports first, then Sports-Reference as a last-resort fallback.")
    if sp.get("errors"):
